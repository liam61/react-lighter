import 'zone.js'
import { asyncManagerSymbol, storeWrappedSymbol, actionWrappedSymbol } from './meta'
...
export default class AsyncManager {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      propKeys.forEach(prop => {
        if (typeof storeInstance[prop] === 'function') {
          storeInstance[prop] = this.wrapStoreFunc(storeInstance[prop], storeInstance, prop)
        }
      })
  ...
  getFunctionKeys(obj) {
    ...
    let propKeys = [...Object.keys(obj), ...Object.getOwnPropertyNames(objProto)]
